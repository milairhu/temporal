...
type nexusCompletionTestCase struct {
	...
	CompletionInfo *persistencespb.ChasmNexusCompletionInfo
...
func (s *schedulerNexusCompletionSuite) TestHandleNexusCompletion_Success() {
	s.runTestCase(&nexusCompletionTestCase{
		...
		CompletionInfo: &persistencespb.ChasmNexusCompletionInfo{
			RequestId: "req-1",
			Outcome: &persistencespb.ChasmNexusCompletionInfo_Success{
...
func (s *schedulerNexusCompletionSuite) TestHandleNexusCompletion_Failure() {
	s.runTestCase(&nexusCompletionTestCase{
		...
		CompletionInfo: &persistencespb.ChasmNexusCompletionInfo{
			RequestId: "req-1",
			Outcome: &persistencespb.ChasmNexusCompletionInfo_Failure{
...
func (s *schedulerNexusCompletionSuite) runTestCase(tc *nexusCompletionTestCase) {
	...
	// Execute the handler.
	err = sched.HandleNexusCompletion(ctx, tc.CompletionInfo)
	s.NoError(err)
	...
	if tc.CompletionInfo.GetSuccess() != nil {
		s.NotNil(lastCompletion.GetSuccess())
	} else if tc.CompletionInfo.GetFailure() != nil {
	...
	s.Empty(invoker.GetWorkflowID(tc.CompletionInfo.RequestId))
